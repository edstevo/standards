...
description: Create concise prompts for PR Agent from rough feature, bug, or refactor requests. Use when a user needs a short codebase-grounded prompt with current implementation, target behavior, and open questions, without creating or updating GitHub issues.
license: MIT
metadata:
  domain: workflow
  role: specialist
  scope: planning
  triggers: PR Agent, pr-agent, prompt, brief prompt, issue prompt, current implementation, target behavior, open questions
---
...
- Do not add plan, scope, risks, affected areas, or key tests sections.
...
- [ ] Write the three-section prompt.
